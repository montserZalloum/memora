...
plans = frappe.get_all(
	"Memora Academic Plan", fields=["name", "title", "is_published"], order_by="modified desc", limit=10
)

for i, plan in enumerate(plans, 1):
	status = "✓" if plan.is_published else "✗"
	print(f"  {i}. {status} {plan.name} - {plan.title}")
	...
	if not plan_subjects:
		...
		for ps in plan_subjects:
			...
			tracks = frappe.get_all(
				"Memora Track", filters={"parent_subject": subject_name}, fields=["name", "title"]
			)
			...
			for track in tracks:
				units = frappe.get_all(
					"Memora Unit", filters={"parent_track": track.name}, fields=["name", "title"]
				)
				...
				for unit in units:
					topics = frappe.get_all(
						"Memora Topic", filters={"parent_unit": unit.name}, fields=["name", "title"]
					)
