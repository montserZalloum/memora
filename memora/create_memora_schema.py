"""
Memora Schema Setup Script
Creates all DocTypes in the correct order following the specifications.
Execute with: bench --site [site-name] eval "exec(open('apps/memora/memora/create_memora_schema.py').read())"
"""
import frappe
from frappe.core.doctype import DocType
...
# 2. Game Plan Subject (Child Table)
create_or_update_doctype({
    "doctype": "DocType",
...
# 3. Game Bundle Content (Child Table)
create_or_update_doctype({
    "doctype": "DocType",
...
# 4. Game Subscription Access (Child Table)
create_or_update_doctype({
    "doctype": "DocType",
