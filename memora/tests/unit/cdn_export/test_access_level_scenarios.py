...
class TestAccessLevelPublic(unittest.TestCase):
	...
	def test_is_published_flag_returns_public(self):
...
class TestAccessLevelPrecedence(unittest.TestCase):
	...
	def test_precedence_set_free_second_highest(self):
		"""Set Free override beats is_free_preview, required_item, inheritance, is_published"""
		node = Mock()
	...
	def test_precedence_is_free_preview_third(self):
		"""is_free_preview beats required_item, inheritance, is_published"""
		node = Mock()
	...
	def test_precedence_required_item_fourth(self):
		"""required_item beats inheritance and is_published"""
		node = Mock()
	...
	def test_precedence_parent_paid_inheritance(self):
		"""Parent paid inheritance beats is_published on child"""
		node = Mock()
	...
	def test_precedence_is_published_fifth(self):
		"""is_published beats default authenticated"""
		node = Mock()
		node.name = "public_vs_auth"
		node.is_published = True
		node.is_free_preview = False
...
class TestAccessLevelCombinations(unittest.TestCase):
	...
	def test_public_child_of_free_preview(self):
		"""Child with is_published under free_preview parent stays public"""
		node = Mock()
		node.name = "public_child_preview"
		node.is_published = True
		node.is_free_preview = False
		node.required_item = None
		# is_published flag overrides parent free_preview
