	...
	def wrapper(*args, **kwargs):
		...
		if not device_id:
			frappe.logger().warning(
				f"[require_authorized_device] GATE 1 FAILED - missing X-Device-ID. "
		...
		authorized_devices = [d if isinstance(d, str) else d.decode("utf-8") for d in raw_members]

		if not device_auth.is_device_authorized(user_id, device_id):
			frappe.logger().warning(
				f"[require_authorized_device] GATE 2 FAILED - device not authorized. "
				f"user={user_id} device_id={device_id} endpoint={fn.__name__} "
				f"authorized_count={len(authorized_devices)}"
			)
			log_security_event(
			...
			frappe.throw(f"Unauthorized device. Please log in again.", frappe.PermissionError)

		session_id = frappe.session.sid
		active_session_bytes = redis_client.get(f"memora:active_session:{user_id}")
		...
		if not session_manager.validate_session(user_id, session_id):
			frappe.logger().warning(
				...
				f"user={user_id} device_id={device_id} endpoint={fn.__name__} "
				f"current_sid={session_id} redis_active_session={active_session}"
			)
			log_security_event(
				...
				redis_active_session=active_session,
			)
			frappe.throw(
...
def login(usr, pwd, device_name=None):
	...
	if not device_id:
		log_security_event(
			event_type="missing_device_id", user_id=usr, device_id=None, endpoint="login", result="rejected"
		)
		frappe.throw("X-Device-ID header required", frappe.PermissionError)
	...
	profile = frappe.db.get_value("Memora Player Profile", {"user": user_id}, ["name", "user"], as_dict=True)
	...
	existing_devices = frappe.get_all(
		"Memora Authorized Device", filters={"parent": profile_name}, fields=["device_id"]
	)
	...
	if len(existing_devices) == 0:
		...
		device_in_db = any(d.device_id == device_id for d in existing_devices)
		if device_in_db:
			redis_client = frappe.cache()
			redis_client.sadd(get_player_devices_key(user_id), device_id)
			frappe.logger().info(f"Device re-cached for {user_id}: {device_id}")
		else:
			log_security_event(
				event_type="unauthorized_device",
				user_id=user_id,
			...
			frappe.throw(
				"Unauthorized device. Contact administrator to authorize this device.", frappe.PermissionError
			)
	else:
...
def check_device_authorization():
	...
	if not profile_name:
		return {"authorized": False, "device_count": 0}

	device_count = len(
		frappe.get_all("Memora Authorized Device", filters={"parent": profile_name}, fields=["name"])
	)

	device_id = frappe.get_request_header("X-Device-ID")
	authorized = device_auth.is_device_authorized(user_id, device_id)

	return {"authorized": authorized, "device_count": device_count}
...
def register_device(student_email, device_id, device_name):
	...
	profile_name = frappe.db.get_value("Memora Player Profile", {"user": student_email}, "name")
...
def complete_lesson(lesson_id, hearts_earned):
	...
	return {
		...
		"lesson_id": lesson_id,
		"hearts_earned": hearts_earned,
		"xp": {"awarded": xp_awarded, "new_total": new_total_xp},
		"streak": {
...
def add_xp(xp_amount):
	...
	return {"message": "XP awarded successfully", "xp_awarded": xp_amount, "new_total_xp": new_total_xp}
