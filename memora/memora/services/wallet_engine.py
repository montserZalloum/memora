...
from memora.utils.redis_keys import (
    get_wallet_key,
    get_pending_wallet_sync_key,
    get_last_played_at_synced_key
)
...
def update_streak(user_id, hearts_earned):
    ...
    frappe.logger().info(
        f"Streak {action} for {user_id}: {current_streak} -> {new_streak}"
    )
...
def add_xp(user_id, xp_amount):
    ...
    frappe.logger().info(
        f"XP awarded to {user_id}: +{xp_amount}, new total: {new_total_xp}"
    )
