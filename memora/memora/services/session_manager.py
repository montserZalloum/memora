...
import frappe
from memora.utils.redis_keys import get_active_session_key
...
def create_session(user_id, device_id, session_id=None):
    ...
    if old_session and old_session != session_id:
        frappe.logger().info(
            f"Session replaced for {user_id}: {old_session} -> {session_id} "
            f"(device: {device_id})"
        )

    frappe.logger().info(
        f"Session created for {user_id}: {session_id} "
        f"(device: {device_id})"
    )
...
def validate_session(user_id, session_id):
    ...
    if not is_valid:
        frappe.logger().info(
            f"Session validation failed for {user_id}: "
            f"expected {active_session}, got {session_id}"
        )
...
def invalidate_session(user_id):
    ...
    if session_id:
        ...
        frappe.logger().info(
            f"Session invalidated for {user_id}: {session_id}"
        )
